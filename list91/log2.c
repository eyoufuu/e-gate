...
inline void log2_updateinslognode(u8 iphash,u8 netseghash,u8 proid,u32 ip,u32 dir,u32 size,u32 pass)
{        	
...
inline void log2_updatestalognode(u8 iphash,u8 netseghash,u8 proid,u32 ip,u32 dir,u32 size,u32 pass)
{        	
    //	node->log.activetime = g_ptm->curtime;
	TLognode* node = log2_getlognode(iphash,netseghash,proid,ip);
	if(unlikely(node == NULL))
...
{
	char buf[200];
    sprintf(buf,"delete from instraffic where ip = 0");
    sql_query("baseconfig",buf);
	sprintf(buf,"insert into instraffic values(%u,0,0,0,0,0)",g_ptm->curtime);
	sql_query("baseconfig",buf);
}
