...
{
	return ReplaceBDB_Len(pmem->dbp,(void*)jhash,hash_len, value,len);
}
...
{
	return RecordBDB_Len(pmem->dbp, jhash,sizeof(CORE_TYPE),value,len);
}
unsigned int replace_mem_hash_core(mem_hash* pmem, CORE_TYPE* jhash,char *value, int len)
{
	return ReplaceBDB_Len(pmem->dbp,jhash,sizeof(CORE_TYPE), value,len);
}
unsigned int query_mem_hash_core(mem_hash* pmem,CORE_TYPE* jhash, char * value, int len)
{
	return QueryBDB_Len(pmem->dbp, jhash,sizeof(CORE_TYPE), value ,len);
}
unsigned int delete_mem_hash_core(mem_hash* pmem,CORE_TYPE* jhash)
{
	DeleteBDB_Len(pmem->dbp ,jhash,sizeof(CORE_TYPE));
}
...
{
	mem_hash * rethash = malloc(sizeof(mem_hash));
	//rethash->dbp  = NULL;
	//rethash->envp = NULL;
	//DB *dbp	      = rethash->dbp;
	...
	open_flags =DB_CREATE| DB_INIT_CDB| DB_INIT_MPOOL|DB_THREAD|DB_PRIVATE;
	...
	//if the db is 2.6.26.8
	ret = rethash->envp->set_flags(rethash->envp, DB_LOG_INMEMORY, 1);
        //higher than 2.6.26.8
//	ret = rethash->envp->set_flags(rethash->envp, DB_LOG_IN_MEMORY, 1);
	if (ret != 0) {
...
inline unsigned long long gethash_value_1(unsigned short tcp_udp,unsigned short sport, unsigned int sip)
{
...
int main(void)
{
	...
	unsigned int x =2;
	unsigned long long key = gethash_value_1(1,90,23456);
	if(record_mem_hash_core(mem,&key,(char*)&x,4)==RET_BDB_FAIL)
		printf("bad record\n");

	x =100;
	if(query_mem_hash_core(mem,&key,(char*)&x,4)==RET_BDB_FAIL)
		printf("bad query\n");
	//pthread_t pt1,pt2;
