 ...
 */
 //以上代码暂时废弃
 //先把所有的项目准备好，然后再选取所有值填进去
 
 $SQL = "select proid,name,type from procat where proid>=0 order by type";
 $result = $db->query2($SQL,"M",false);
 $responce = array();
 foreach($result as $row)
 {
 ...
 $SQL = "select webid,name,type from webcat where webid>=0 order by type";
 $result = $db->query2($SQL,"M",false);
 foreach($result as $row)
 {
