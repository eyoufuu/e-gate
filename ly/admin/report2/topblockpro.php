...
<?php
   ...
   if ($systemmode == 0)
     ...
     switch($symbol)
    {
    	case "1":
         $show= "全部";   
    	 $arr = "select v.name, u.blocknum from (select pro_id , sum(packets_blocked_num) as blocknum from $tablename where logtime between $dates and $datee group by pro_id order by blocknum desc limit 0 ,9) as u left join (select proid,name from procat) as v on u.pro_id=v.proid";
         break;
    	case "2":
    		 ...
    		 if ($systemmode == 0)
            {
		      ...
		      $arr = "select v.name, u.blocknum from (select pro_id ,sum(packets_blocked_num) as blocknum from $tablename where (ip_inner=$accountid) and  (logtime between $dates and $datee) group by pro_id order by blocknum desc limit 0 ,9) as u left join (select proid,name from procat) as v on u.pro_id=v.proid";
            } 
           else
            {
		      ...
		      $arr = $db->fetchRow($arr);
		      $accountid= $arr["id"];
		      $arr = "select v.name, u.blocknum from (select pro_id ,sum(packets_blocked_num) as blocknum from $tablename where (account_id=$accountid) and  (logtime between $dates and $datee) group by pro_id order by blocknum desc limit 0 ,9) as u left join (select proid,name from procat) as v on u.pro_id=v.proid ";
             }     
    	  break;
    	case "3":
    	...
    	case "4":
			...
			$format = "select v.name,u.blocknum from (select pro_id,sum(packets_blocked_num) as blocknum from %s where (ip_inner between  %u and %u) and (logtime between %u and %u ) group by pro_id order by blocknum desc limit 0 ,9) as u left join (select proid,name from procat) as v on u.pro_id=v.proid";
