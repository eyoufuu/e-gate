<?php
	...
	switch($symbol)
    {
    	case "1":
    	    if ($systemmode == 0)
            {
			  $SQL = "call keywordip_all('$tablename',$dates,$datee)";
		    }
           else
		     { 
               $SQL = "call keywordaccount_all('$tablename',$dates,$datee)";
    		 } 
    		break;
    	case "2":
    		...
    		if ($systemmode == 0)
              {
		        ...
		        $accountid= $arr["bindip"];
				 //$format = "call keywordip_id('%s',%u,%u,%u)"; 
			     //$SQL=sprintf($format,'$tablename',$dates,$datee,$accountid); 		
			   ...
			   $SQL = " call keywordip_id('$tablename',$dates,$datee,$accountid)";
              }   		
          else
              {
			 ...
			 //$SQL="select u.id,u.logtime,w.account as name,u.ip_inner,u.host,u.note as keyword,u.pass from (select id, FROM_UNIXTIME(logtime) as logtime,ip_inner,account_id,host,note,pass from $tablename where (account_id=$accountid) and (get_type=3) and (logtime between $start and $datee)  limit  $start,$limit) as u  left join (select id,account from useraccount) as w on u.account_id=w.id ";
			   $SQL = " call keywordaccount_id('$tablename',$dates,$datee,$accountid)";
    		  }
   	   break;
    	case "3":
		       ...
		       $accountid= $arr["id"];    		 
			 //$SQL="select u.id,u.logtime,w.account as name,u.ip_inner,u.host,u.note as keyword,u.pass from (select id, FROM_UNIXTIME(logtime) as logtime,ip_inner,account_id,host,note,pass from $tablename where (account_id=$accountid) and (get_type=3) and (logtime between $start and $datee)  limit  $start,$limit) as u  left join (select id,account from useraccount) as w on u.account_id=w.id ";
			   $SQL = "call keywordaccount_id('$tablename',$dates,$datee)";      		
    	...
    	case "4":
			 ...
			 if ($systemmode == 0)
                 ...
                 
				{
			     ...
			     $format = "call keywordip_ips('%s',%u,%u,%u,%u)"; 
			     $SQL=sprintf($format,$tablename,$dates,$datee,$ipsi,$ipei);
                  //$SQL="call keywordip_ips('$tablename',$dates,$datee,$ipsi,$ipei)"; 			  				 
			    }
    	  		else
	...
	$aColumns = array('logtime', 'name', 'ip_inner', 'host','keyword','pass');
