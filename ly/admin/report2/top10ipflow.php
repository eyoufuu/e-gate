    ...
    switch($symbol)
    {
    	...
    	case "2":
            ...
            } else
            {
                  ...
                  $arr = $db->fetchRow($arr);
		          $accountid= $arr["id"];
                  $arr = "select v.account as name,INET_NTOA(u.ip) as ip,u.upflow,u.downflow,u.updownflow from (select ip_inner as ip,floor(sum(upflow)/1024) as upflow,floor(sum(downflow)/1024) as downflow, floor(sum(upflow+downflow)/1024) as updownflow from  $tablename where (account_id=$accountid) and (logtime between $dates and $datee) group by account_id order by updownflow desc limit 0 ,9) as u left join  (select id,account from useraccount) as v on u.account_id=v.id";
    	...
    	case "3":
		    ...
		    $arr = $db->fetchRow($arr);
		    $accountid= $arr["id"];
		    $arr = "select v.account as name,INET_NTOA(u.ip) as ip,u.upflow,u.downflow,u.updownflow from (select ip_inner as ip,account_id,floor(sum(upflow)/1024) as upflow,floor(sum(downflow)/1024) as downflow, floor(sum(upflow+downflow)/1024) as updownflow from $tablename where (account_id=$accountid) and (logtime between $dates and $datee) group by account_id order by updownflow desc limit 0 ,9 ) as u  left join (select id,account from useraccount) as v on u.account_id=v.id ";
...
<html>
   ...
   <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
	...
	<tbody>
	  ...
	  ?>
		<tr>
	        <td class="left"><?php echo "<a href='#' onclick='get(&quot;". $value['ip'] ." &quot;)'>" . $value['ip'] ."</a>" ?></td>
