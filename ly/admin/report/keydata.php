...
<?php
   ...
   if ($systemmode == 0)
	 ...
	 switch($symbol)
    {
    	...
    	case "4":
			 if ($systemmode == 0)
               $format=" select u.id,u.logtime,w.name,u.ip_inner,u.host,u.note as keyword,u.pass from (select id, FROM_UNIXTIME(logtime) as logtime,ip_inner,account_id,host,note,pass from %s where (ip_inner between u% and u%) and (get_type=3) and (logtime between %u and %u) limit %u,%u) as u left join (select name,bindip from useraccount) as w on u.ip_inner=w.bindip ";              
