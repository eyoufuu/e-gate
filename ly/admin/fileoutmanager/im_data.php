...
<?php
	...
	$SQL  = "SELECT * FROM  file_transter where function='即时通讯' order by $sidx $sord LIMIT $start,$limit";
	...
	foreach($result as $row){
		...
		if($row['block']==0)
		  $box = "<input type = checkbox checked id = '" .$row[id] . "im' />";
       	else
		  $box = "<input type = checkbox id = '" .$row[id] . "im' />";
